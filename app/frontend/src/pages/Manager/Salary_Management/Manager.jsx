...
function Manager() {
  ...
  const fetchSalaries = async () => {
    try {
      const res = await axios.get("http://localhost:3000/displaySalaries");
      setUsers(res.data);
    } catch (error) {
  ...
  const deleteSalary = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/deleteSalary/${id}`);
  ...
  const showFile = (fileName) => {
    window.open(`http://localhost:3000/files3/${fileName}`, "_blank", "noreferrer");
  };
