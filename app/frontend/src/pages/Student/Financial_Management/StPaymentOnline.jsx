...
function StPaymentOnline() {
  ...
  useEffect(() => {
    axios.get('/studentprofile')
      .then((res) => setStudentid(res.data.stdid))
  ...
  const submit = (e) => {
    e.preventDefault();
    const paymentAmount = parseFloat(subamount || 0);
    const updatedBalance = parseFloat(balance || 0) - paymentAmount;
  ...
  const handleSubmit = async (e) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({ title: "Payment Canceled", icon: "error" });
        }
      });
  };
